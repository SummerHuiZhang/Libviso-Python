...
int main (int argc, char** argv) {
  ...
  for (int32_t i=0; i<4541; i++) {
    ...
    std::cout<<left_img_file_name<<std::endl;
    ...
    try {
      ...
      if (viso.process(left_img_data,dims)) {
        ...
        vector<Matcher::p_match>  matched_feature = viso.getMatches ();        // output some statistics
        Matrix feature3d = viso.getFeature3D();
        double num_matches = viso.getNumberOfMatches();
        double num_inliers = viso.getNumberOfInliers();
        cout << ", Matches: " << num_matches<< " "<<matched_feature.size()<<" "<<feature3d.n;
      ...
      myfile<<Matrix::reshape(pose.getMat(0,0,2,3),1,12)<<endl;
